...
<html lang="en" dir="ltr">
  ...
  <body><br><br><br><br>
    <div class="container">
      <h2>Cari Pesanan</h2>
        silahkan isi formulir pencarian dibawah ini dengan memasukan nomor telp yang anda gunakan untuk pemesanan
        <br><br>
      <form class="form-group" action="{{route('Invoice.store')}}" method="post">
        @csrf
        <input type="text" placeholder="Masukan Nomor Telp" name="telp" class="form-control" required value="">

      <div class="mt-3">
        <input type="submit" name="submit" value="Cari Pesanan" class="btn btn-success ">

        <a href="{{route('Bantuan.index')}}" class="nav-item ml-3">Batal</a>
      </div>
      </form>
    </div>
  </body>
